...
Usage:
    python skills/idea-generation/scripts/novelty_check.py --idea "Adaptive attention pruning via gradient importance"
    python skills/idea-generation/scripts/novelty_check.py --idea "..." --max-rounds 10
    python skills/idea-generation/scripts/novelty_check.py --idea-file outputs/my-topic/top_idea.json
"""
...
def generate_search_queries(idea: str) -> list[str]:
    ...
    for word in idea.split():
        if len(word) > 3 and word.lower() not in {
            "with", "from", "that", "this", "using", "based", "through",
            "which", "their", "have", "been", "into", "also", "more",
        }:
...
def run_novelty_check(idea: str, max_rounds: int = 5, result_limit: int = 10) -> dict:
    ...
    print("Checking novelty of idea:")
        ...
        if papers and round_num < max_rounds and not search_queries:
            for paper in papers[:2]:
                ...
                if title and len(title.split()) >= 3:
                    search_queries.append(title[:80])

    ranked = sorted(
        all_papers_seen.values(),
        key=lambda paper: paper.get("citationCount", 0),
        reverse=True,
    )
    ...
    for index, paper in enumerate(ranked[:5], 1):
        print(
            f"  {index}. [{paper.get('year', '?')}] {paper.get('title', '?')} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Check research idea novelty via Semantic Scholar"
    )
    parser.add_argument("--idea", type=str, help="Research idea to check (text)")
    parser.add_argument(
        "--idea-file",
        type=str,
        help="JSON file containing idea (must have 'Title' or 'Experiment' field)",
    )
    parser.add_argument(
        "--max-rounds", type=int, default=5, help="Max search rounds (default: 5)"
    )
    parser.add_argument(
        "--result-limit",
        type=int,
    ...
    if args.idea_file:
        with open(args.idea_file, encoding="utf-8") as file:
            idea_data = json.load(file)
